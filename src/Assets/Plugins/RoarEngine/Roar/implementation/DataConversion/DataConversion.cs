...
namespace Roar.implementation.DataConversion
{
	...
	public class XmlToFriendInvite : IXmlToObject<DomainObjects.FriendInvite>
	{
		public XmlToFriendInvite ()
		{
		}
		
		public string GetKey (IXMLNode n)
		{
			return n.GetAttribute("invite_id");
		}
		
		public FriendInvite Build(IXMLNode n)
		{
			FriendInvite friend_invite = new FriendInvite();
			friend_invite.invite_id = n.GetAttribute("invite_id");
			friend_invite.player_id = n.GetFirstChild("player_id").Text;
			return friend_invite;
		}
	}
	
	public class XmlToFriendInviteInfo : IXmlToObject<DomainObjects.FriendInviteInfo>
	{
		public string GetKey (IXMLNode n)
		{
			...
			if (from_node == null)
				return "";
			return from_node.GetAttribute("player_id");
		}
		
		public FriendInviteInfo Build (IXMLNode n)
		{
			FriendInviteInfo info = new FriendInviteInfo();
			IXMLNode from_node = n.GetFirstChild("from");
			...
			if (from_node != null)
			{
				...
				{
					throw new InvalidXMLElementException("Unable to parse level to integer");
				}
				info.friend_invite_row_id = from_node.GetAttribute("friend_invite_row_id");
			}
